...
class FloatingRasterNewFormat
{
	...
	var frameBuffer:Int;
	var width:Int;
	var vertex_mapper:VertexMapper;
	
	public function new( call:Dynamic, frameBuffer:Int, width:Int, domain:ByteArray, vertex_mapper:VertexMapper, ?debug:Bool ) 
	{
		this.frameBuffer = frameBuffer;
		this.width = width;
		this.vertex_mapper = vertex_mapper;
	...
	function end( ctx:format.abc.Context )
	{
		...
		var cops = untyped ctx.curFunction.ops;
		
		var s:String = StringTools.replace( cops.toString( ), ",O", "\nO" );
	...
	function bytecode( ctx:format.abc.Context )
	{
		...
		var rast = new RasterizerFloat( __abc__, ctx, frameBuffer );
		
		var poly:Dynamic = rast.read_vertices_map( vertex_mapper );
	...
	function scanline_flat( x0:Reg<Float>, x1:Reg<Float>, y:Reg<Float> )
	{
	...
	function scanline_debug( x0:Reg<Float>, x1:Reg<Float>, y:Reg<Float> )
	{
		...
		OElse,	
			...
			plotPoint( x0, y, 0xFF333333 );
			plotPoint( x1, y, 0xFFFFFFFF );
		__abc__([
		OFi,	
		]);
	}
