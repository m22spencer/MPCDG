...
class SerpenskiGen3DFloatingNewFormatRoutine
{
	...
	public function new() 
	{
		...
		bmdToTex( new CubeTex256x256( ), 199000000 );
		
		
		vmap = new XYZ20Step( );
		pix_shader = new OverdrawShader( );
		
		new FloatRenderer( preinit, ba, 0, width, new LumShader( ), vmap );
	}
	
	function preinit( shade_debug:FloatRendererClassDef )
	{
		shader_debug = shade_debug;
		new FloatRenderer( init, ba, 0, width, pix_shader, vmap );
	}
	...
	function drawCube( x:Float, y:Float, z:Float, cubewidth:Float )
	{
		...
		{Memory.setFloat( pr, x+hw );
		 ...
		 Memory.setFloat( pr+12, cast 63 );
		 Memory.setFloat( pr+16, cast 127 );
		 pr += 20;
		
		 Memory.setFloat( pr, x-hw );
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z-hw );
		 Memory.setFloat( pr+12, cast 0 );
		 Memory.setFloat( pr+16, cast 127 );
		 pr += 20;
		...
		{Memory.setFloat( pr, x+hw );
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z+hw );
		 Memory.setFloat( pr+12, cast 128 );
		 Memory.setFloat( pr+16, cast 128 );
		 pr += 20;
		
		 Memory.setFloat( pr, x-hw );
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z+hw );
		 Memory.setFloat( pr+12, cast 191 );
		 Memory.setFloat( pr+16, cast 128 );
		 pr += 20;
		...
		{Memory.setFloat( pr,   x-hw );
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z-hw );
		 Memory.setFloat( pr+12, cast 255 );
		 Memory.setFloat( pr+16, cast 127 );
		 pr += 20;
		 ...
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z+hw );
		 Memory.setFloat( pr+12, cast 192 );
		 Memory.setFloat( pr+16, cast 127 );
		 pr += 20;
		...
		{Memory.setFloat( pr,   x+hw );
		 ...
		 Memory.setFloat( pr+4, y-hw );
		 Memory.setFloat( pr+8, z+hw );
		 Memory.setFloat( pr+12, cast 127 );
		 Memory.setFloat( pr+16, cast 127 );
		 pr += 20;
		...
		{Memory.setFloat( pr,   x+hw );
		 ...
		 Memory.setFloat( pr+8, z+hw );
		 Memory.setFloat( pr+12, cast 127 );
		 Memory.setFloat( pr+16, cast 191 );
		 pr += 20;
	...
	function init( shade:FloatRendererClassDef )
	{
		shader = shade;
		cur_shader = shade;
		canvas = new BitmapData( width, width, false );
		buffer = new BitmapData( width, width, false );
	...
	{	
		...
		time = Lib.getTimer( );
		var pixels:Int = cur_shader.shade( m_start + offset, m_start + offset + offset );
